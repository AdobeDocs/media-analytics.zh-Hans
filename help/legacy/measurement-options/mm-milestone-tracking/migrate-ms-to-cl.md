...
视频测量的核心概念与里程碑和自定义链接跟踪相同，后者采用视频播放器事件并将其映射到分析方法，同时捕获播放器元数据和值并将它们映射到分析变量。 “自定义链接”方法应被视为对实施和收集的数据进行了精简和简化。 使用“自定义链接”解决方案，不会为视频测量预定义变量或方法，需要进行完全自定义设置。 也可以更新播放器事件代码，以指向基本播放器事件（例如开始和结束）的自定义链接跟踪调用。 请参阅[《自定义链接实施指南》](/help/legacy/measurement-options/cl-in-aa/cl-impl-guide.md)，以了解更多详细信息。
