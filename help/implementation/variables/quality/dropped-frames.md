...
| **XDM集合字段** | [`xdm.mediaCollection.qoeDataDetails.droppedFrames`](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/xdm/data-types/qoe-data-details-collection) |
...
调用[`sendEvent`](https://experienceleague.adobe.com/cn/docs/experience-platform/collection/js/commands/sendevent/overview)时，在`xdm.mediaCollection.qoeDataDetails`中设置`droppedFrames`：
...
调用`sendMediaEvent`时，在`xdm.mediaCollection.qoeDataDetails`中设置`droppedFrames`：
...
调用`xdm.mediaCollection.qoeDataDetails`中包含`droppedFrames`的[bitrateChange](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/bitratechange/#bitratechange)终结点：
...
将累积丢帧计数作为第四个参数传递给`ADBMobile.media.createQoSObject`并更新跟踪器：
...
将累积丢帧计数作为第四个参数(`droppedFrames`)传递给`adb_media_init_qosinfo`，并使用`mediaUpdateQoS`更新跟踪器：
