...
| **XDM集合字段** | [`xdm.mediaCollection.sessionDetails.album`](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/xdm/data-types/session-details-collection) |
...
调用[`sendEvent`](https://experienceleague.adobe.com/cn/docs/experience-platform/collection/js/commands/sendevent/overview)时，在`xdm.mediaCollection.sessionDetails`中设置`album`：
...
调用`xdm.mediaCollection.sessionDetails`中包含`album`的[sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart)终结点：
...
在调用`trackSessionStart`之前，使用`ADBMobile.media.AudioMetadataKeys.ALBUM`在媒体对象的`StandardMediaMetadata`属性中设置相册：
...
在调用`mediaTrackSessionStart`之前，使用`MEDIA_AudioMetadataKeyALBUM`在媒体对象的标准元数据中设置相册：
