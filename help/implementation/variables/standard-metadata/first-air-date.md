...
| **XDM集合字段** | [`xdm.mediaCollection.sessionDetails.firstAirDate`](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/xdm/data-types/session-details-collection) |
| **Audience Manager特征** | `c_contextdata.a.media.airDate` |
| **必需** | 否 |
...
调用[`sendEvent`](https://experienceleague.adobe.com/cn/docs/experience-platform/collection/js/commands/sendevent/overview)时，在`xdm.mediaCollection.sessionDetails`中设置`firstAirDate`：
...
调用`xdm.mediaCollection.sessionDetails`中包含`firstAirDate`的[sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart)终结点：
...
在调用`trackSessionStart`之前，使用`ADBMobile.media.VideoMetadataKeys.FIRST_AIR_DATE`设置媒体对象的`StandardMediaMetadata`属性中的首次播放日期：
...
调用`mediaTrackSessionStart`之前，使用`MEDIA_VideoMetadataKeyFIRST_AIR_DATE`在媒体对象的标准元数据中设置首次播放日期：
