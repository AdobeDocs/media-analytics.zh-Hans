...
**广告**&#x200B;维度报告每个播放的唯一广告，由`media.adStart`上设置的广告ID作为密钥。 维度是广告报表的主要细分，是广告级别分类（如广告名称、广告长度和Creative ID）的联接键。

## 如何填充此维度

广告由播放器在每个`media.adStart`事件中设置为广告的稳定标识符。
...
每个项目都是在`media.adStart`上报告的唯一广告ID。 为每个创意内容使用稳定的标识符，以便同一广告可以跨会话汇总为单个行项目。
