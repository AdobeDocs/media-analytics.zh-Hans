...
在收到[会话结束](/help/implementation/events/session/session-complete.md)事件时，媒体后端设置`mediaReporting.sessionDetails.isCompleted = true`。 该量度在结束调用时报告。 在没有显式`sessionComplete`的情况下超时的会话不计为完成。

| 报告系统 | 来源 |
| --- | --- |
| Adobe Analytics | 启用[[!UICONTROL 媒体核心]](/help/reporting/media-reports-enable.md)后，自动从上下文数据`a.media.complete`收集。 |
| Customer Journey Analytics | [`mediaReporting.sessionDetails.isCompleted`](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/xdm/data-types/session-details-reporting) |
