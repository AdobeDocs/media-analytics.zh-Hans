...
媒体后端在每个picture-in-picture state-start事件上递增`mediaReporting.states[]`的`pictureInPicture`条目中的`count`字段。 该量度在结束调用时报告。
...
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/xdm/data-types/media-reporting-details)条目，其中`name = "pictureInPicture"`，字段`count` |
