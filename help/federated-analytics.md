...
* **Media Analytics 实施：**&#x200B;发件人必须在将成为联合数据集一部分的所有播放器上实施 Media Analytics。只有 Media Analytics 数据可用于联合。请参阅文档：[测量Adobe Analytics的流媒体](/help/media-overview.md)

* **Adobe 咨询合同：**&#x200B;在初始设置发送者与接收者之间的联合规则时，该合同对于与咨询服务部合作来审查数据和创建数据共享协议，非常有价值。

## 下载 Federated Analytics 表单

要参加Federated Analytics，请下载并填写[联合规则协议](federated-analytics-form.pdf)表。
