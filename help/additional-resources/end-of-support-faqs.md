...
# Media Analytics Mobile SDK终止支持常见问题解答

2021年8月31日终止支持版本4 Mobile SDK，届时，Adobe还将终止对适用于iOS和Android的Media Analytics Mobile SDK的支持。 2021年8月31日之后，Adobe将不会提供Media Analytics Mobile SDK的修复、与操作系统相关的更新或支持。  在迁移到这些新 Experience Platform SDK 的过程中，请牢记，必须实施 [Media Analytics 扩展](https://aep-sdks.gitbook.io/docs/using-mobile-extensions/adobe-media-analytics)，才能启用 Adobe Analytics for Streaming Media。
...
1. Analytics for Streaming Media 实施需要安装 AEP Mobile SDK 以及使用 Analytics 扩展和 Media Analytics 扩展。从 2021 年 9 月 1 日起，您应使用新的 AEP Mobile SDK 和扩展。Media Analytics扩展是使用Adobe标记（数据收集）配置的。  有关更多信息，请参阅[从独立的 Media SDK 迁移到 Adobe Launch](/help/legacy/sdk-to-launch/sdk-to-launch-migration.md)

1. 已终止适用于 iOS 和 Android 的 Media Analytics SDK 的功能开发。从 2019 年秋季开始引入的新功能均通过 Media Analytics 扩展和媒体收集 API 启用。

1. Roku 和 Chromecast SDK 仍可供 Analytics for Streaming Media 客户使用。Roku SDK 和 Chromecast SDK 将继续作为独立的 SDK 进行增强和支持。如果您使用适用于Media Analytics的JS SDK，则可以继续使用独立的SDK或使用Adobe数据收集启用Media Analytics扩展(以前称为AdobeLaunch)。
...
1. **终止支持是否会影响适用于 tvOS 的 SDK？**
   ...
   是，对于 tvOS（版本 10+），建议的实施是迁移到 Media Analytics 扩展。有关更多信息，请参阅[从独立的 Media SDK 迁移到 Adobe Launch - iOS](/help/legacy/sdk-to-launch/sdk-to-launch-migration-platforms/sdk-to-launch-migration-ios.md)。

1. **终止支持是否会影响适用于Fire TV和AndroidTV的SDK?&#x200B;**

   是，对于Fire TV和AndroidTV，建议的实施是迁移到Media Analytics扩展。  有关更多信息，请参阅[从独立的 Media SDK 迁移到 Adobe Launch - Android](/help/legacy/sdk-to-launch/sdk-to-launch-migration-platforms/sdk-to-launch-migration-android.md)。
